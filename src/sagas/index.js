...
function* workerSaga() {
  try {
    ...
    const uniquePatientsByMonth = transformedData.reduce((s, n) => {
    ...
    const uniquePatientsByWeek = transformedData.reduce((s, n) => {
      const { patient_id, start_date_time } = n;
      const d = new Date(start_date_time);
      const day = days[d.getDay()];
      const item = s.find(i => i.day === day);
    ...
    const totalFeesByMonth = transformedData.reduce((s, n) => {
    ...
    const totalFeesByWeek = transformedData.reduce((s, n) => {
      const { completed_fees, start_date_time } = n;
      const d = new Date(start_date_time);
      const day = days[d.getDay()];
      const item = s.find(i => i.day === day);
